...
class AutoPigeon {
  ...
  static clone(doc, historyLength=HISTORY_LENGTH) {
    ...
    meta.get(clone).gids = new Set(meta.get(doc).gids);
    return clone;
  }
  ...
  static rewindChanges(doc, ts, cid) {
    ...
    while (true) {
      ...
      if (change.ts > ts || (change.ts == ts && change.cid > cid)) {
        ...
        meta.get(doc).gids.delete(c.gid);
  ...
  static fastForwardChanges(doc) {
    ...
    while (change = stash.pop()) {
      ...
      meta.get(doc).gids.add(change.gid);
      history.push(change);
    }
  }
  ...
  static applyChanges(doc, changes) {
    ...
    if (meta.get(doc).gids.has(changes.gid)) {
      return newDoc;
    }
    try {
    ...
    try {
      ...
      meta.get(newDoc).gids.add(changes.gid);
    } catch (e) {
